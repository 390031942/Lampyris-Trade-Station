...
// Project Include(s)
#include "QStyledGroupBox.h"

void QStyledGroupBoxInternal::paintEvent(QPaintEvent* e) {
...
void QStyledGroupBoxInternal::mouseMoveEvent(QMouseEvent* e) {
	int buttonCount = this->m_icons.size();
...
void QStyledGroupBoxInternal::mousePressEvent(QMouseEvent* e) {
	int buttonCount = this->m_icons.size();
...
void QStyledGroupBoxInternal::leaveEvent(QEvent* e) {
	this->m_hoveredMask &= 0xffff0000;
	this->update();
}

QStyledGroupBoxInternal::QStyledGroupBoxInternal(DisplayMode mode, const QVector<QPixmap>& icons,
...
QStyledGroupBox::QStyledGroupBox(const QVector<QPixmap>& icons, const QVector<QString>& infos) :
	m_button(new QStyledGroupBoxInternal(QStyledGroupBoxInternal::Both, icons, infos)),
	...
	QObject::connect(this->m_button, &QStyledGroupBoxInternal::signalSelectedIndexChanged, [=](int i) {
		emit this->signalSelectedIndexChanged(i);
	});
}